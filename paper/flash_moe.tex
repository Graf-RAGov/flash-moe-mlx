...
\title{\textbf{Flash-MoE: Streaming a 397B Parameter Mixture-of-Experts Model\\ from NVMe at 5.5 Tokens/Second on Consumer Hardware}}
...
% ============================================================================
\begin{abstract}
We demonstrate that a 397 billion parameter Mixture-of-Experts language model (Qwen3.5-397B-A17B) can run on a laptop with only 48\,GB of unified memory at 5.5 tokens per second with production-quality output.
...
Key innovations include a fused three-command-buffer GPU pipeline that eliminates CPU--GPU synchronization overhead, BLAS-accelerated linear attention for GatedDeltaNet layers, and direct SSD access with \texttt{F\_NOCACHE} bypassing the page cache.
We achieve 5.5\,tok/s sustained and 7+\,tok/s peak on an Apple M3 Max, reaching 97\% of the theoretical I/O-limited maximum.
...
\begin{enumerate}[leftmargin=*,itemsep=1pt]
    ...
    \item An empirical analysis of 90 experiments spanning the optimization trajectory from 0.28\,tok/s to 5.68\,tok/s, documenting both successful and failed approaches.
\end{enumerate}
...
\paragraph{F\_NOCACHE direct access.}
We set \texttt{fcntl(fd, F\_NOCACHE, 1)} on all expert file descriptors.
This bypasses the kernel page cache entirely, avoiding two problems: (1)~120\,GB of expert data would thrash the page cache, evicting useful operating system data, and (2)~the page cache adds a memcpy from kernel to user buffers that is unnecessary when we immediately DMA the data to GPU-visible memory.
Direct access yields consistent 5.68\,tok/s vs.\ variable performance with cached I/O.

\paragraph{LRU Expert Cache.}
We optionally maintain an LRU cache of 500 recently-loaded experts (3.5\,GB) in Metal shared memory.
Cache hits avoid SSD reads entirely; cache misses trigger \texttt{pread()}.
In practice, consecutive tokens share 30--50\% of experts across layers, yielding hit rates of 35--50\%.
However, the cache's DRAM footprint competes with the operating system's file buffer cache and Metal's GPU working set.
Our best configuration uses \emph{no} application-level cache, relying on \texttt{F\_NOCACHE} direct I/O for consistent throughput.
...
The journey from first working inference (0.47\,tok/s with MLX) to our final Metal engine (5.68\,tok/s) represents a 12$\times$ improvement.
...
\midrule
\textbf{2-bit experts} & \textbf{2} & \textbf{0} & \textbf{5.55} & \textbf{7.05} & \textbf{3.1} & \textbf{Excellent} \\
...
The remaining 41.5\,GB is available for the operating system, other applications, and the kernel's file buffer cache (which provides implicit read-ahead and caching for the expert files in non-\texttt{F\_NOCACHE} mode).
...
\textbf{Flash-MoE} & \textbf{4.4$\times$} & \textbf{5.5} & M3 Max 48\,GB \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
    ...
    \item \textbf{Experiments 72--90} (2-bit + polish): Requantization, BLAS acceleration, fused pipelines. Final: 5.68\,tok/s.
\end{enumerate}
...
Qwen3.5-397B-A17B---a model with 397 billion total parameters---runs at 5.5 tokens per second on a laptop with 48\,GB of RAM, using only 6.5\,GB of resident memory.
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
    ...
    \item Direct SSD access (\texttt{pread} + \texttt{F\_NOCACHE}) outperforms memory-mapped I/O by 5$\times$ for large, non-sequential reads.
...
Our system achieves 97\% of the I/O-limited theoretical maximum, suggesting that further gains require either faster storage or reduced I/O through better quantization.
...
84 & keep & 5.68 & 5.5 & F\_NOCACHE + 2-bit: final best (5.68 avg, 5.91 peak) \\
\bottomrule
\end{tabular}
\end{table*}
